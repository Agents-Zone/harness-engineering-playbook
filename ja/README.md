...
[AgentsZone](https://agentszone.ai)は、AIエンジニアリングプラクティスに特化した中国語開発者コミュニティです。メンバーは独立系開発者、スタートアップCTO、大手テック企業の現場エンジニアなど多様であり、共通点はただ一つ——全員が実際のプロジェクトでAI Agentを本格的に活用してコードを書き、ソフトウェアをデリバリーしていることです。本書は数百回にわたるプラクティス共有から蒸留されたもので、すべての結論が複数の実践者によって検証されています。

> **注意: 初期ドラフト** 本書はアーキテクチャ検証段階にあります。序文と目次構成はおおむね固まっており、全体的なフレームワークに対するフィードバックを歓迎します。各章はまだ詳細、具体例、読みやすさの磨き込みが必要です。実践者コントリビューターによる章ごとの改善を歓迎します。
...
しかし、デリバリーデータは別の物語を語る。PRの数は増え、レビュー時間は長くなり、バグ率は上昇する。モデルはより強力で、ツールはより優れ、体感は速くなっている——にもかかわらず、全体の生産性は追いついていない。

一方で、同じツールを使いながらまったく異なる成果を出している人々がいる——生産性向上は数十倍単位で、デリバリーされたコードは本番環境にデプロイされ、実戦で検証済みだ。

1.5倍にしかならないという実感も正しいし、数十倍を達成して本番に投入したという話も正しい。同じモデル、同じツールなのに、このギャップはどこから生まれるのか。AgentsZoneコミュニティで100人以上の開発者と交わした対話が、明確な答えを与えてくれた。本書は、1.5倍から100倍へ至る方法論とエンジニアリングプラクティスを体系的にまとめたものである。
...
1.5倍と100倍のギャップは、正確にはどこにあるのか。プロンプト技法でもなく、ツール選定でもなく、モデル能力でもない——これらはオペレーションレベルの話であり、同じツールが異なるチームの手で根本的に異なる結果を生む理由を説明できない。

違いは規律から生まれる。100倍チームはAI Agentの特性に合ったエンジニアリングシステムを構築している。1倍チームは人間の実践者向けに設計されたシステムのままAgentを動かしている。
...
本書のすべての章の背後には、実際のプラクティショナーがいます。[コントリビューターページ](contributors.md)に名前を連ねてくださった皆さん——事例を提供し、誤りを正し、漠然としたアイデアを実行可能な方法へと磨き上げてくださいました。同様に、コミュニティでの議論で質問を投げかけ、苦い経験を共有しながらも名前を残さなかった方々にも感謝します。皆さんのプラクティスこそが、本書の最も重要な原材料です。
