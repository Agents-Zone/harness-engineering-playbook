...
原则和机制都已经清楚。这一节用 ch4 建立的博客审核队列的载体走一遍完整流程。前半段展示三层机制怎么从载体派生加人判定失效边界建起来。后半段展示建成的机制跑一遍五份产出，抓到三种漂移。整个流程沿用 4.3 的共同产出框架。
...
候选检查项摆出来之后，人的介入在失效边界。三层机制每一层的能力上限画在哪里，是人要判断的。判定沿用 4.3 三条件（领域、杠杆、呈现形式）。判定内容从意图变成失效边界，接口原则相同。
...
三个判定都在 4.3 三条件的判定框架内跑通。判定的是失效边界，接口原则相同。
...
覆盖检查通过不是终点。它意味着按照当前对漂移的理解，三层机制的覆盖是完整的。真正跑起来之后可能发现意料之外的漂——原来不知道能这样漂的地方，机制就没准备。这类发现进入演进循环，属于下一章。5.4 里的失效边界文档是初始建立时的产出，是运行的起点，不是终态。
...
两处介入都在共同产出的框架内跑。都符合 4.3 的三条件判定。差别只在时间点和判定内容。建立时判定的是失效边界，运行时判定的是载体新增决策。同一个机制的两次使用。
...
到这里 ch5 的论证闭环。载体和验证机制本身怎么持续维护是下一章的话题。产品会变，模型会升级，新场景会暴露旧机制的漏。这些是演进要处理的。
